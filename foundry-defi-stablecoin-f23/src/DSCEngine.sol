...
contract DSCEngine is ReentrancyGuard {
    ...
    event DSCMinted(address indexed user, uint256 indexed amountMinted);
    event CollateralRedeemed(
        address indexed redeemedFrom, address indexed redeemedTo, address indexed token, uint256 amount
    );
    event Liquidated(address indexed liquidator, address indexed liquidated);
    ...
    * @param tokenCollateralAddress: The ERC20 token address of the collateral you're depositing
        ...
        nonReentrant
    {
        ...
        _revertIfHealthFactorIsBroken(msg.sender);
        emit Liquidated(msg.sender, user);
    }
    ...
    function getCollateralTokens() public view returns (address[] memory tokens) {
        tokens = s_collateralTokens;
    }

    function getCollateralBalanceOfUser(address user, address collateral) external view returns (uint256 amount) {
        amount = s_collateralDeposited[user][collateral];
    }

    function getCollateralTokenPriceFeed(address token) external view returns (address priceFeedAddress) {
        priceFeedAddress = s_priceFeeds[token];
    }
}
