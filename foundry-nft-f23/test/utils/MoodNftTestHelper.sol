...
contract MoodNftTestHelper {
    ...
    string public s_baseUri = "data:application/json;base64,";

    function getNftURIFromImage(string memory imageUri, string memory contractName) public returns (string memory tokenURI) {
        tokenURI = string(
            abi.encodePacked(
                s_baseUri,
                Base64.encode(
